...
if __name__ == '__main__':
    ...
    Height = IntVar(menu)
    global Exposure;
    Exposure = IntVar(menu)
    global Fps;
    Fps = IntVar(menu)
    global nImagesInit;
    nImagesInit = IntVar(menu)
    global TimeInit;
    TimeInit = DoubleVar(menu)
    global TimeAcquisition;
    TimeAcquisition = DoubleVar(menu)
    global switch_grid;
    switch_grid = BooleanVar(menu);
    global gridvar1;
    gridvar1 = IntVar(menu)
    global gridvar2;
    gridvar2 = IntVar(menu)
    global invert_camera;
    invert_camera = IntVar(menu)
    global k_snapshot;
    k_snapshot = IntVar(menu);
    global amplifyRight;
    amplifyRight = BooleanVar(menu);
    global amplifyLeft;
    amplifyLeft = BooleanVar(menu);
    global TypeImg;
    TypeImg = StringVar(menu);
    global preview_test;
    preview_test = BooleanVar(menu)
    ...
    selection_btn = tk.Button(menu, text='Select folder', width=19, height=1, bg='#DADDE3', activebackground='#ccd9e1',
                              fg='#282C34',
                              command=lambda: module.folder(console, folder_status, selectionFolder, k_snapshot))
    ...
    tk.Button(menu, text='Preview', width=19, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: module.preview(menu, preview_test, visualize_status, console, Width, Height, Exposure,
                                             Fps, amplifyLeft, amplifyRight,
                                             canvas_left, canvas_right, gridvar1, gridvar2, invert_camera)).place(
        x=xinit, y=yinit + dyinit * 12 + 23 * 2)
    ...
    tk.Button(menu, text='Snapshot', width=20, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: module.snapshot(console, selectionFolder, preview_test, k_snapshot, TypeImg,
                                              invert_camera)).place(x=xinit + 1306, y=yinit + dyinit * 9 + 23 * 2)
    ...
    tk.Button(menu, text='Start capture', width=20, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: module.start_capture(menu, console, visualize_status, preview_test, Width, Height,
                                                   Exposure, Fps, selectionFolder, TypeImg,
                                                   canvas_left, canvas_right, nImagesInit, TimeInit, TimeAcquisition,
                                                   invert_camera)).place(x=xinit + 1306, y=yinit + dyinit * 10 + 23 * 2)
    ...
    tk.Button(menu, text='Stop capture', width=20, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: module.stop_capture(menu, console, selectionFolder, folder_status)).place(x=xinit + 1306,
                                                                                                        y=yinit + dyinit * 11 + 23 * 2)
    ...
    tk.Button(menu, text='Amplify L', width=10, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: Thread(target=module.amplifyL, args=(preview_test, amplifyLeft)).start()).place(
        x=xinit + 722, y=yinit + dyinit * 8 + 23 * 2)
    ...
    tk.Button(menu, text='Amplify R', width=10, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: Thread(target=module.amplifyR, args=(preview_test, amplifyRight)).start()).place(
        x=xinit + 1368, y=yinit + dyinit * 8 + 23 * 2)
